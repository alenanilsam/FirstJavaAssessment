...
public class assesmentexcel {
	...
	public static void main(String[] args) throws IOException {
		...
		if(value==1) {
			...
			assesmentexcel hi=new assesmentexcel();
			if(adm==0) {
				hi.main(args);
			}
			else {
		...
		/*try  
		{  
		File file = new File("C:\\Users\\alen.anil\\Downloads\\23.xlsx");   //creating a new file instance  
	    ...
	    ListIterator<Functioncalling> iterator =null;
	    Functioncalling student = new Functioncalling();
		Scanner scanner=new Scanner(System.in);
		double admissionNumber=scanner.nextInt();
		...
		for (int i = 0; i < r.getLastCellNum() + 1; i++) {
            ...
            if(i==2) {
            	student.setPhysicsMark(cell.getNumericCellValue());
            }
            if(i==3) {
            	student.setChemistryMark(cell.getNumericCellValue());
            }
            if(i==4) {
		...
		for(Functioncalling student1 :studentList)
		{
			double totalMark=student.getphysicsMark()+student.getchemistryMark()+student.mathematicsMark();
			double percentage=(totalMark/300)*100;
			int choice=0;
			gradeAssignment grading=new gradeAssignment();
			boolean found=false;
			...
			do {
				...
				case 2:
					...
					while(iterator.hasNext()) {
						...
						for( Functioncalling studentdata:studentList) {
							if(studentdata.getAdmission()==adm){
								for( Functioncalling studentdata1:studentList) {
									System.out.println("AdmissionNumber:"+studentdata.getAdmission()+"\r\n"+"Name :"+studentdata.getName()+"\r\n"+"Name:"+studentdata.getphysicsMark());
								    System.out.println("totalMark");
								}
							}
